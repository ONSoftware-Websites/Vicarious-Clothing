...
function newsletterHtml(subject: string, preheader: string, body: string) {
...
${bodyHtml}
<p style="margin:24px 0 0;font-size:12px;color:#8b8b93;">You’re receiving this because you signed up at ${SITE_URL}. <a href="${SITE_URL}/account/preferences" style="color:#007587;">Unsubscribe</a> anytime.</p>
</td></tr>
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const html = newsletterHtml(String(subject), String(preheader || ""), String(body));

    if (testEmail) {
      const res = await fetch("https://api.resend.com/emails", {
        method: "POST",
        headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ from, to: String(testEmail), subject: String(subject), html }),
      });
      if (!res.ok) {
        const text = await res.text();
        return Response.json({ error: `Resend failed: ${res.status} ${text}` }, { status: 500 });
      }
      await logEmail({ to: String(testEmail), subject: String(subject), template: "newsletter", status: "sent", provider: "resend", sentAt: new Date().toISOString(), preview: String(subject) });
    ...
    // Batch send (Resend allows up to 100 per request with `to` array, but we send individually for personalization + logging)
    let sent = 0;
    const errors: string[] = [];
    // Throttle to ~5 concurrent to avoid rate limits
